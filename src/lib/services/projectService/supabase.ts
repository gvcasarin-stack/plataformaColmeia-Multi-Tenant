...
export const getProjectsWithFilters = async (filters: {
  ...
  try {
    ...
    // Colunas JSONB pesadas (timeline_events, documents, files, comments, history) são
    // excluídas desta query de lista — são carregadas sob demanda ao abrir um projeto.
    let query = supabase
      .from('projects')
      .select(`
        id,
    ...
    const projects: Project[] = data.map(item => {
      ...
      return {
        ...
        planta_situacao_config: item.planta_situacao_config || undefined,
