...
export function EditPackageModal({ open, onOpenChange, package: pkg, onSuccess }: EditPackageModalProps) {
  ...
  const { register, handleSubmit, formState: { errors }, reset, setValue } = useForm<PackageFormData>();

  // Pré-preencher formulário quando o pacote mudar
  useEffect(() => {
    if (open && pkg) {
      setValue('nome', pkg.nome);
      setValue('quantidade_projetos', pkg.quantidade_projetos);
      setValue('valor', pkg.valor);
      setValue('validade_dias', pkg.validade_dias);

      // Configurar potência máxima
      const potencia = (pkg as any).potencia_maxima_kwp;
  ...
  const onSubmit = async (data: PackageFormData) => {
    ...
    try {
      ...
      devLog.log('[EditPackageModal] Atualizando pacote:', pkg.id, payload);
      ...
      if (!result.success || !response.ok) {
        throw new Error(result.error || 'Erro ao atualizar pacote');
      }

      toast({
        title: 'Pacote atualizado!',
        description: `O pacote "${data.nome}" foi atualizado com sucesso`,
      });
      ...
      devLog.error('[EditPackageModal] Erro ao atualizar pacote:', error);
      toast({
        title: 'Erro ao atualizar pacote',
        description: error.message || 'Ocorreu um erro ao atualizar o pacote',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Atualize as informações do pacote de projetos
          </DialogDescription>
        </DialogHeader>
