...
export async function GET(request: NextRequest) {
  try {
    ...
    devLog.log('🧪 TESTANDO FUNÇÃO SQL initialize_new_organization')
    ...
    devLog.log('📋 Funções encontradas:', functions)
    
    if (funcError) {
      devLog.error('❌ Erro ao buscar funções:', funcError)
    ...
    devLog.log('🚀 Testando função com dados fictícios...')
    ...
    if (testError) {
      devLog.error('❌ Erro no teste da função:', testError)
      return NextResponse.json({
    ...
    devLog.log('✅ Teste da função bem-sucedido:', testResult)
    ...
    devLog.error('💥 Erro inesperado no teste:', error)
    return NextResponse.json({ 
