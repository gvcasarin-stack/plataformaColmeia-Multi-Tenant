...
export async function POST(request: NextRequest) {
  try {
    ...
    devLog.log('[API notifications/mark-read] Headers multi-tenant:', {
      tenantId,
    ...
    if (process.env.NODE_ENV === 'development' && tenantId && (tenantId.startsWith('temp-') || tenantId.startsWith('fallback-'))) {
      devLog.log('[API notifications/mark-read] Tenant temporário detectado (dev), simulando sucesso:', tenantId);
    ...
    if (process.env.NODE_ENV === 'production' && tenantId && (tenantId.startsWith('temp-') || tenantId.startsWith('fallback-'))) {
      devLog.error('[API notifications/mark-read] ERRO CRÍTICO: Tenant temporário em produção!', { tenantId });
      return NextResponse.json({
    ...
    if (!finalUserId) {
      devLog.log('[API notifications/mark-read] userId não fornecido, tentando obter da sessão...');
      ...
      finalUserId = user.id;
      devLog.log('[API notifications/mark-read] userId obtido da sessão com sucesso');
    }
