-- ============================================
-- Script: Popular campos SLA em projetos existentes - HOMOLUX SOLAR
-- Descrição: Preenche campos de SLA (status_changed_at, sla_expires_at, sla_expired)
--            para todos os projetos existentes da organização Homolux Solar
...
BEGIN
    ...
    RAISE NOTICE '📊 Total de projetos Homolux: %', v_total_projects;
...
WITH project_status_config AS (
    ...
    WHERE ps.tenant_id = '923589ed-f5e5-4b94-a5d6-6e2a43f8dc5b'
),
projects_to_update AS (
    ...
    WHERE p.tenant_id = '923589ed-f5e5-4b94-a5d6-6e2a43f8dc5b'
...
LEFT JOIN projects p ON p.status = ps.slug
    ...
    AND p.tenant_id = '923589ed-f5e5-4b94-a5d6-6e2a43f8dc5b'
WHERE ps.tenant_id = '923589ed-f5e5-4b94-a5d6-6e2a43f8dc5b'
...
-- 1. Faça login como usuário da Homolux Solar
...
-- 5. Compare com os resultados da Solar Tech para garantir consistência
